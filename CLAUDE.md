...
4. On finalize, `apps/web/src/lib/meeting-insights.ts` runs `detectMeetingAssistInsights` once and writes `MeetingInsight` rows. Real-time assist is **deterministic — no model call**. (Speaker diarization of the "others" channel is separate and **opt-in** via `MEETING_CAPTURE_DIARIZATION`: when enabled it runs a local voice-embedding model per finalized utterance to assign a `Speaker N` label — see `docs/features/meeting-assistant.md`.)
